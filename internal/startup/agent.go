...
import (
	...
	"github.com/kenmoini/seesaw-engine/internal/agent"
	"github.com/kenmoini/seesaw-engine/internal/logging"
)

// RunAgent will run the Seesaw Agent
func (config Config) RunAgent() {
	...
	server := &http.Server{
		...
		Handler:      agent.NewRouter(config.Seesaw.Server.BasePath),
